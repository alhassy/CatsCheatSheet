...
# Since the initial object, if it exists, is unique up to unique isomorphism
An alternative notation for $⦇ B ⦈$ is $\text{!`}_B$.
...
we obtain: $γ$ is a colimit for $D$ if there is a mapping $γ╲-$ such that /╲-Type/ and
/╲-Char/ hold.

#+BEGIN_EXPORT latex
\begineqns

\eqn{╲-Type}{ δ \text{ cocone for } D \impliesS γ╲δ : \src\, γ → \tgt\,δ}

\eqn{╲-Char}{ y ﹔ \const{x} = δ \equivS x = γ ╲ δ }
...
Take $D$ and $𝒟$ as suggested by $D𝒟 = \left( •^A \;\; •^B \right)$.
