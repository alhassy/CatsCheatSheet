...
\eqn{$\slash$-Type}{ δ \text{ cocone for } D \impliesS δ╱γ : \tgt\, γ → \tgt\,δ}
...
* COMMENT 2colimits
 ...
 + {Axiom, colimit-Naturality?}{  $x : A \to B \impliesS x \;_{D}\!\fcmp\; \gamma_B \eqs \gamma_A$}

 + {Axiom, colimit-Characterisation}{$\gamma \fcmp_\K x \eqs \delta \equivS x \eqs \gamma \under \delta $}
 + {colimit-Self}{$\gamma \fcmp_\K \gamma \under \delta \eqs \delta $}
 + {colimit-Identity}{$\Id_{CoLimD} \eqs \gamma \under \gamma$}
 + {$\under$-Uniqueness, ``Jointly Epic''}{$\gamma \fcmp_\K x \eqs \gamma \fcmp_\K y \impliesS x \eqs y$}
 + {colimit-Fusion, ``Composition into Numerator''}{$(\gamma \under \delta) \fcmp_\K x \eqs \gamma \under (\delta \fcmp_\K x)$}
