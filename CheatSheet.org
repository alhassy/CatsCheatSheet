...
and $(-→-) : 𝒞ᵒᵖ × 𝒞 → 𝒮ℯ𝓉$ is the hom-functor such that $(f → g) h = g ∘ h ∘ f$.
...
#+BEGIN_EXPORT latex
\eqn{Set-Bridge}{ f ∘ \const{c} = \const{f \, c}}
#+END_EXPORT
...
\eqn{$\slash$-Type}{ δ \text{ cocone for } D \impliesS γ╲δ : \tgt\, γ → \tgt\,δ}
...
\eqn{$\slash$-Char}{ \const{x} ∘ γ = δ \equivS x = δ ╱ γ }
...
\eqn{$\slash$-Self-Cancellation}{ \const{δ╱γ} ∘ γ = δ}

\eqn{$\slash$-Id}{γ╱γ = \Id}

\eqn{$\slash$-Fusion}{x ∘ δ╱γ = (\const{x} ∘ δ) ╱ γ}
% Direct Proof:
%
%    γ╲δ ﹔ x = γ ╲ (δ ﹔ _x_)
...
\eqn{$\slash$-Unique}{\const{x} ∘ γ = \const{y} ∘ γ \impliesS x = y}
...
\eqn{$\slash$-Functor-Dist}{F(δ╱γ) = Fδ ╱ Fγ}

\eqn{$\slash$-Pre-Functor-Elim}{δF╱γF = δ╱γ}
...
\eqn{$\backslash$-Type}{ δ \text{ cone for } D \impliesS γ ╲ δ : \src\, δ → \src\,γ}
...
** §3 FIX
 0. §3.1, page 68, exercise 3.3: The arguments to the ~for~ are in the wrong order; moreover the notation
    for multiplication is inconsistent with in (3.6).

 0. §3.3, page 79: The top-most code fragment mentions ~Eval T~ but nothing in the surrounding text mentions
    ~Eval~. Is this a typo?

 0. §3.4, page 83: In the fourth equation in the bottom-most calculation, there seems to be a missing
    ~cons~ tag on the right-hand side --c.f. (3.27)--. The right-most should be /Cons(a, x)/ instead of just /(a,x)/.

 0. §3.6, page 87: Right after (3.41), there is an inconsistency with the notation of multiplication, or there lack of.
    Perhaps consider using a × or *, rather than just a space.

 0. §3.9, page 96, equation (3.63): Do you mean ${}_n C_p$ instead of ${}^n C_p$?
 ...
 0. §3.9, page 97, exercise 3.10: At the end of the first line, you want $(1 + X)²$ rather than $(1 + X²)$.

 0. §3.12, page 99: When you first introduced isos back in §2, you said they were functions that were both
 ...
 0. §3.15, page 110: First sentence after the functor instance should include a definite article; it should be
    /in the language's/.

 0. §3.16, page 115, line 10: Typo, /more that two/ should be /more than two/.

 0. §3.19, page 123, last paragraph, third line: Typo /such as discipline/ should be /such a discipline/.
...
{\bf Musa Al-hassy} \hfill {\today}
\\
\centerline{\large\textbf{Category Theory Theorem List}}
...
** Environments
 ...
 (\todo Examples of common or intersting categories ; or
 constructions to obtain new categries, here )

 Our common environments will be the following distributive lattices
 \vspace{-1em}
 \begin{itemize}\itemsep-0.3em
  \item Booleans: $(\BB, \implies, \land, \lor, \false, \true)$ {\hfill\color{red}---Our ambient logic!}
  \item Extended Number Line:  $(\RR, \leq, \min, \max, -\infty, +\infty)$ 
  \item Naturals under division: $(\NN, \ | \ , \gcd, \lcm, 1, 0)$
...
** colimits
 ...
 Colimits are precisely initiality, and generalise
 sums, coequalisers, and pushouts.
 ...
 `Cones' sit upright on their base $(D)$ on a table;
 `CoCones` sit upright on a co-table ;)

 \NTHM{}{Axiom, CoCone Category $\bigvee D$}{
   objects are `cocones' $(C : Obj_\mathcal{A},\, \gamma : D \natTo \K{C})$
   and morphisms $f : (C, \gamma) \to (C', \delta)
   \equivS f : C \to C' \lands \gamma \fcmp \K{f} \eqs \delta $
 }

 \def\under{ \backslash }

 A \emph{colimit} for $D$ is an initial object in $\bigvee D$.
 That is, \emph{there exists} a cocone $(CoLimD, \gamma)$ of $D$ and
 a function $\gamma \under -$ such that
 ---copy and paste initiality properties---

 The fraction notation better suggests the \emph{calculational}
 properties.

 ( Maybe call the object $\bigsqcup D$ ? Since it is similar to the `join'
 and a cocone is a `proof' that an object is an upper bound? )

 Let $x\; _F\!\fcmp_G\; y \eqs F\,x \,\fcmp\, G\,y$,
 ---note that $(\eta \fcmp_K x)_A \eqs \eta_A \fcmp x$---
 then:

 \NTHM{}{Axiom, colimit-Definition}{  $x : A \to B \impliesS x \;_{D}\!\fcmp\; \gamma_B \eqs \gamma_A$}

 \NTHM{}{Axiom, colimit-Typing}{$(C, \delta) \text{ cocone for } D \impliesS \gamma \under \delta : CoLimD \to C $}

 \NTHM{}{Axiom, colimit-Characterisation}{$\gamma \fcmp_\K x \eqs \delta \equivS x \eqs \gamma \under \delta $}

 \NTHM{}{colimit-Self}{$\gamma \fcmp_\K \gamma \under \delta \eqs \delta $}

 \NTHM{}{colimit-Identity}{$\Id_{CoLimD} \eqs \gamma \under \gamma$}

 \NTHM{}{$\under$-Uniqueness, ``Jointly Epic''}{$\gamma \fcmp_\K x \eqs \gamma \fcmp_\K y \impliesS x \eqs y$}

 \NTHM{}{colimit-Fusion, ``Composition into Numerator''}{$(\gamma \under \delta) \fcmp_\K x \eqs \gamma \under (\delta \fcmp_\K x)$}

 In accordance with our ``dangerous convention'', we've omitted the
 well-formedness condition that $\delta$ is an object in $\bigvee D$;
 the notation $\cdots\under\delta$ is only senseful if
 $\delta : D \natTo \K{CoLimD}$ ---ie $D x \fcmp \delta_{\tgt \, x} = \delta_{\src \, x}$---, like in arithmetic where the notation $m / n$ is only
 senseful if $n$ differs from 0.

 The following law confirms the choice of notation once more.

 \NTHM{}{$\under$-Compose}{$(\gamma \under \delta) \fcmp (\delta \under \epsilon) \eqs \gamma \under \epsilon$}

 \NTHM{}{$\under$-Functor}{$F(\gamma \under \delta) \eqs F\gamma \under F\delta$}

 \alert{where} $\delta$ and $\epsilon$ are $D$ cocones
 and anything appearing to the left of $\under$ being a colimit!

 Let $\eta F$ be the natural transformation $x \mapsto \eta_{F\, x}$.
 Let $\gamma$ and $\gamma F$ be colimits for the same diagram, then

 \NTHM{}{$\under$-natural transformation}{$\gamma F \under \delta F \eqs \gamma \under \delta$}

 Let $()_x : \varnothing \natTo \K{x}$ be the natural transformation from the
 empty functor $\varnothing : \mathbf{0} \to \mathcal{A}$ to the constant functor.

 \NTHM{}{Initiality as colimit, $\under-\initial{B}\;$}{$ \initial{B} \eqs ()_0 \under ()_B $}
 ...
 \NTHM{}{Sum as colimit, $\under-[]$}{$ [f,g] \eqs (\mathsf{inl}, \mathsf{inr}) \under (f,g)$}

 Needless to say, my favourite:

 \NTHM{}{Colimit as Adjunction}{$ (D \natTo \K C) \;\cong\; (CoLimD \to C)$}
...
** Prelims: skolem et al
 \kern3ex
 \heading{Prelims}

 \heuristic[Skolemisation]
 Assertions of the form ``$\forall\, x \spot \exists!\, y \spot P\, x\, y$''
 are not very well suited for \emph{algebraic manipulation}:
 it is the existential quantification inside the scope of the universal one that
 hinders effective calculation. However, such an assertion is equivalent to
 \centerline{``there is a function $\langle-\rangle$ with $\forall x,y \spot P\,x\,y \equivs y = \langle\, x \, \rangle$''}
 where the existence claim is brought to a global level and so reasoning need no longer be interrupted
 by the declaration and naming of the existence of a unique $y$ that depends on $x$; it can be denoted just $\langle\, x \, \rangle$.
 A variety of bracketing notation may be used for the important role of the various unique $y$'s
 with the additional advantage that no extra parentheses are needed for composite arguments $x$
 (which are expected to occur often).
 As usual, the universal quantification in the above displayed formula are omitted, thus simplifying
 the formulation once more.

 With such a characterisation in-hand, natural questions come to mind.
 ``Self'': Is $\langle\, x \, \rangle$ itself a solution to the problem? Indeed we have
 $P\, x\, \langle\, x \, \rangle$.
 ``Uniqueness'': Is the solution to the problem unique? Indeed, $P\, x\, y \lands P\, x\, z \implies y = z$.
 ``Identity'': Is there a way to use the identity morphisms to truthify the left side?
 ``Fusion'': Can we solve for `?' in $\langle\, x \, \rangle \fcmp f = \langle\, ? \, \rangle$,
 or perhaps the dual composition?

 \emph{How do prove such assertions}: begin with the given side $P\,x\,y$ and try to simplify this
 via a chain of equivalences to obtain $y = \text{``an expression not involving $y$''}$ and
 then define $\langle\, x \, \rangle$ to be the right-hand side of the previous equation.
 Sometimes, equivalences are difficult and we can instead prove, by circular implication,
 that $P\,x\,y \implies y = \cdots  \lands y = \cdots \implies P\,x\,y$
 ---either way, the proof is given as a calculation.
 That is, aim to rewrite $y$ in-terms of $x$ in the given $P\,x\,y$.
...
** Constants
 ...
 Let C be a nonempty data domain and let and /c ∈ C/. Then we define the
...
** Epics and Monics
 ...
 they lose “some” information, which is regarded as uninteresting for some
 ...
 be “blind” enough to confuse different inputs, by mapping them onto the same
...
** Isos
 ...
 one “format”, say /A/, to another format, say /B/, without losing information. 

 So /f/ and /f⁻¹/ are faithful protocols between the two formats /A/ and /B/. Of course,
 these formats contain the same “amount” of information, although the same data
 adopts a different “shape” in each of them. In mathematics, one says that /A/ is
 isomorphic to /B/ and one writes /A ≅ B/ to express this fact.

 Isomorphic data domains are regarded as “abstractly” the same. Note that, in
...
** Sums
 ...
 The gluing is specfied by the equation: /h = [f , g]  ≡  h . inl = f ∧ h . inr = g/
 Read, “/h/ is the sum of /f/ and /g/ precisely when it behaves like /f/ when following a left
 injection and behaves like /g/ when following a right injection.”
 ...
 *?*
 ...
 first kind of “glue” which comes to mind when programmers need to combine, or
 chain functions (or processes) to obtain more elaborate functions (or processes).
 This is because of one of its most relevant properties, associtivity: /(f · g) · h = f · (g · h)/.
