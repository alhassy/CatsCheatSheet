...
There is a function /f˘/ called its “inverse” or “converse”
such that for any functions $x, y$
\eqn{inverse-Char₁}{ f ∘ x = y \equivS x = f˘ ∘ y}

This immediately yields,

\eqn{inverse-Cancellation}{ f ∘ f˘ = \Id \landS f˘ ∘ f = \Id}

From which we regain the other `shunting rule'
\eqn{inverse-Char₂}{ x ∘ f = y \equivS x = y ∘ f˘}

If, for some reason, $f˘$ is found handier than isomorphism $f$
in reasoning, then the shunting rules can be of help.
