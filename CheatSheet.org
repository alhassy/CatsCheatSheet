...
\eqn{$\slash$-Unique ; JointlyEpic}{\const{x} ∘ γ = \const{y} ∘ γ \impliesS x = y}
...
\eqn{$\backslash$-Unique ; JointlyMonic}{γ ∘ \const{x} = γ ∘ \const{y} \impliesS x = y}
...
* Monoidal and Closed Categories
  ...
  \newline
  $\_{}⊗\_{} : 𝒞^2 → 𝒞$ and object /I : Obj 𝒞/ and three natural isomorphisms:
  \newline
  The ‘associator’ /α_{A, B, C} : A ⊗ (B ⊗ C) ≅ (A ⊗ B) ⊗ C/ and
  \newline
...
:End:
  ...
  is categorical product; i.e., it's a category with finite products and exponentials.
