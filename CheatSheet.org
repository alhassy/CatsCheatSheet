...
is a 𝒞-object, and a morphism from $γ$ to $δ$ is a 𝒞-morphism $x$ such that $\const{x} ﹔ γ = δ$.
...
\eqn{/-Fusion}{\const{x} ﹔ δ ╱ γ = (\const{x} ﹔ δ) ╱ γ}
...
\ref{/-Unique} expresses that colimits γ have an monic-like property: \\
